...
var File = function(mtime, content, encoding) {
  this.mtime = mtime;
  ...
  this.getBuffer = function() {
    return new Buffer(this.content, this.encoding);
  };
};
...
var Mock = function(structure) {
  ...
  this.writeFile = function(filePath, content, encoding, callback) {
    predictableNextTick(function() {
      ...
      iCallback = callback || encoding;   // assumes that the given is a function

      if (pointer && typeof pointer === 'object' && !(pointer instanceof File)) {
        pointer[baseName] = new File(0, content, iEncoding);
        iCallback(null);
      } else {
